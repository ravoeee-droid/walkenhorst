...
begin
  ...
  if p_outcome not in ('no_answer','reached','interested','meeting','callback','not_interested','wrong_number') then
  ...
  if p_outcome in ('not_interested') then
    update public.energy_campaign_members
    set status='stopped',stopped_reason='call_not_interested',updated_at=now()
    where lead_id=v_lead.id and status='queued';
  end if;

  if p_outcome in ('no_answer','callback','interested') then
    v_due := case
      when p_outcome='callback' then coalesce(p_callback_at,now()+interval '1 day')
    ...
    values(
      v_user,
      v_lead.id,
      case p_outcome when 'callback' then v_lead.company_name||' zurückrufen' when 'interested' then v_lead.company_name||' – Interesse nachfassen' else v_lead.company_name||' erneut anrufen' end,
      v_due,
      v_priority,
      'open',
      nullif(left(coalesce(p_note,v_title),500),'')
    );
  end if;

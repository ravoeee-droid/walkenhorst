...
declare v_message public.energy_messages%rowtype;
begin
  ...
  insert into public.energy_email_events(message_id,event_type,url,user_agent) values(v_message.id,p_event,p_url,left(p_user_agent,500));
  update public.energy_leads set intent_score=least(100,intent_score + case when p_event='click' then 15 else 5 end),updated_at=now() where id=v_message.lead_id;
  return true;
end $$;
...
create or replace function public.energy_store_mailbox_secrets(p_mailbox_id uuid,p_user_id uuid,p_smtp_password text,p_imap_password text default null)
returns boolean language plpgsql security definer set search_path = public,vault as $$
declare v_smtp_id uuid; v_imap_id uuid; v_existing_smtp uuid; v_existing_imap uuid;
begin
  ...
  if p_smtp_password is not null and length(p_smtp_password)>0 then
    if v_existing_smtp is null then select vault.create_secret(p_smtp_password,'energy_smtp_'||p_mailbox_id::text,'Walkenhorst SMTP password') into v_smtp_id;
    else perform vault.update_secret(v_existing_smtp,p_smtp_password); v_smtp_id:=v_existing_smtp; end if;
  else v_smtp_id:=v_existing_smtp; end if;
  if p_imap_password is not null and length(p_imap_password)>0 then
    if v_existing_imap is null then select vault.create_secret(p_imap_password,'energy_imap_'||p_mailbox_id::text,'Walkenhorst IMAP password') into v_imap_id;
    else perform vault.update_secret(v_existing_imap,p_imap_password); v_imap_id:=v_existing_imap; end if;
...
returns table(smtp_password text,imap_password text) language sql security definer set search_path = public,vault as $$
 select (select decrypted_secret from vault.decrypted_secrets where id=m.smtp_secret_id),(select decrypted_secret from vault.decrypted_secrets where id=m.imap_secret_id) from public.energy_mailboxes m where m.id=p_mailbox_id and m.user_id=p_user_id limit 1
$$;

create or replace function public.energy_get_system_secret(p_name text)
returns text language sql security definer set search_path=public,vault as $$ select decrypted_secret from vault.decrypted_secrets where name=p_name limit 1 $$;

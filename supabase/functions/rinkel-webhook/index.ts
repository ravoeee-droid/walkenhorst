...
async function processEvent(db:any,userId:string,p:any){
 ...
 if(!call){
   const direction=type==="outgoing"?"outgoing":type==="incoming"?"incoming":"unknown";const counterpart=direction==="outgoing"?phone(p?.to):direction==="incoming"?phone(p?.from):null;
   if(counterpart){const {data:lead}=await db.from("energy_leads").select("id").eq("user_id",userId).eq("phone_e164",counterpart).limit(1).maybeSingle();leadId=lead?.id||null}
 ...
 if(leadId){
   ...
   if(type==="outgoing")await db.from("energy_leads").update({last_contact_at:occurred,status:["new","research","ready"].includes(String((await db.from("energy_leads").select("status").eq("id",leadId).single()).data?.status||""))?"contacted":undefined,updated_at:new Date().toISOString()}).eq("id",leadId).eq("user_id",userId);
   if(type==="insights"&&String(p?.sentiment||"").toUpperCase()==="POSITIVE")await db.from("energy_intent_events").insert({user_id:userId,lead_id:leadId,source:"rinkel",event_type:"positive_call_sentiment",weight:15,external_id:`rinkel:${externalId}:insights`,metadata:{summary:p?.summary||null,topics:p?.topics||[]}}).then(()=>undefined);
 }
}

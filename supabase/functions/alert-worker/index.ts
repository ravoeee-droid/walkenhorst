...
Deno.serve(async(req)=>{
 ...
 try{
  if(!await authorize(req,db))return out({error:"Nicht autorisiert"},401);let created=0,failed=0,recovered=0;const now=new Date();const overdue=new Date(now.getTime()-15*60_000).toISOString();const since24=new Date(now.getTime()-24*3600_000).toISOString();const since7=new Date(now.getTime()-7*86400_000).toISOString();
  const[fups,mailboxes,integrations,outboxRows,calls,bounces,bounceAlerts]=await Promise.all([
    ...
    db.from("energy_mailboxes").select("id,user_id,email_address,status,last_error,last_bounce_error").limit(1000),
    ...
    db.from("energy_alerts").select("id,user_id,metadata").eq("alert_type","hard_bounce_spike").eq("status","open").limit(1000),
  ]);
  const unhealthyMailboxes=(mailboxes.data||[]).filter((m:any)=>m.status==="error"||Boolean(m.last_error)||Boolean(m.last_bounce_error));
  const healthyMailboxes=(mailboxes.data||[]).filter((m:any)=>m.status==="ready"&&!m.last_error&&!m.last_bounce_error);
  const work:Promise<void>[]=[];
  for(const f of fups.data||[])work.push(alert(db,f.user_id,f.lead_id,"hot_followup_overdue","hot",`${f.title}: SLA überfällig`,f.reason||"Hot Follow-up liegt länger als 15 Minuten unbearbeitet.",`followup:${f.id}:overdue`,new Date().toISOString(),{followup_id:f.id,due_at:f.due_at}).then(()=>{created++}).catch(()=>{failed++}));
  for(const m of unhealthyMailboxes)work.push(alert(db,m.user_id,null,"mailbox_error","critical",`${m.email_address}: Mailbox prüfen`,m.last_bounce_error||m.last_error||`Status ${m.status}`,`mailbox:${m.id}:error`,new Date().toISOString(),{mailbox_id:m.id}).then(()=>{created++}).catch(()=>{failed++}));
  ...
  const bounceMap=new Map<string,{userId:string;mailboxId:string;count:number}>();
  for(const b of bounces.data||[]){const k=`${b.user_id}:${b.mailbox_id}`;const cur=bounceMap.get(k)||{userId:b.user_id,mailboxId:b.mailbox_id,count:0};cur.count++;bounceMap.set(k,cur)}
  for(const item of bounceMap.values()){if(item.count<3)continue;work.push(alert(db,item.userId,null,"hard_bounce_spike","hot","Hard-Bounce-Spike erkannt",`${item.count} Hard Bounces in den letzten 24 Stunden auf einer Mailbox. Versandgesundheit prüfen.`,`bounce:${item.mailboxId}:${new Date().toISOString().slice(0,10)}`,new Date().toISOString(),{mailbox_id:item.mailboxId,count_24h:item.count}).then(()=>{created++}).catch(()=>{failed++}))}
  await Promise.all(work);
  for(const a of bounceAlerts.data||[]){const mailboxId=String(a.metadata?.mailbox_id||"");if(!mailboxId)continue;const current=bounceMap.get(`${a.user_id}:${mailboxId}`)?.count||0;if(current>=3)continue;const{data,error}=await db.from("energy_alerts").update({status:"done",resolved_at:new Date().toISOString(),updated_at:new Date().toISOString()}).eq("id",a.id).eq("status","open").select("id");if(error)failed++;else recovered+=(data||[]).length}
  for(const m of healthyMailboxes){const{data,error}=await db.from("energy_alerts").update({status:"done",resolved_at:new Date().toISOString(),updated_at:new Date().toISOString()}).eq("user_id",m.user_id).eq("alert_type","mailbox_error").eq("status","open").contains("metadata",{mailbox_id:m.id}).select("id");if(error)failed++;else recovered+=(data||[]).length}
  return out({ok:true,signals:work.length,alerts_upserted:created,alerts_recovered:recovered,failed});

...
export function RevenueAnalytics({user}:{user:User}){
  ...
  return <main className="os-root" style={{minHeight:"100vh"}}><div className="os-content" style={{maxWidth:1400,margin:"0 auto",paddingTop:28}}>
    ...
    <section className="os-card os-section" style={{marginTop:18}}><div className="os-section-head"><div><div className="os-kicker">Rinkel Call Performance</div><h2>Anrufen → Gespräch → Termin</h2></div><a className="os-btn" href="/calls">Caller Queue →</a></div><div className="os-grid" style={{gridTemplateColumns:"repeat(5,minmax(0,1fr))",gap:10}}>{[[outgoingCalls.length,"Ausgehende Calls","Rinkel"],[answeredCalls.length,"Beantwortet",`${answerRate}% Answer Rate`],[positiveCalls.length,"Positive Calls","AI Sentiment"],[`${avgCallMinutes} Min.","Ø Gespräch","beantwortete Calls"],[`${callToMeeting}%","Calls → Termin+",`${callLeadsMeetingPlus}/${callLeadIds.size} Call-Leads`]].map(([v,l,s])=><div className="os-callout" key={String(l)}><small>{l}</small><strong style={{display:"block",fontSize:27,marginTop:6}}>{v}</strong><span style={{fontSize:9,color:"#8792a4"}}>{s}</span></div>)}</div></section>

...
export function RevenueAnalytics({user}:{user:User}){
  ...
  const [deals,setDeals]=useState<Deal[]>([]);const [events,setEvents]=useState<StageEvent[]>([]);const [campaigns,setCampaigns]=useState<Campaign[]>([]);const [messages,setMessages]=useState<Message[]>([]);const [leads,setLeads]=useState<Lead[]>([]);const [error,setError]=useState<string|null>(null);
  const load=useCallback(async()=>{if(!supabase)return;const[d,e,c,m,l]=await Promise.all([
  ...
  ]);if(d.error){setError(d.error.message);return}setDeals((d.data||[]) as Deal[]);if(!e.error)setEvents((e.data||[]) as StageEvent[]);if(!c.error)setCampaigns((c.data||[]) as Campaign[]);if(!m.error)setMessages((m.data||[]) as Message[]);if(!l.error)setLeads((l.data||[]) as Lead[])},[supabase,user.id]);
  ...
  return <main className="os-root" style={{minHeight:"100vh"}}><div className="os-content" style={{maxWidth:1400,margin:"0 auto",paddingTop:28}}>
    <div className="os-section-head"><div><div className="os-kicker">Revenue Intelligence</div><h1 className="os-title">Was wird Umsatz – und wo verlieren wir ihn?</h1><p>Deal-History, Forecast, Conversion und Kampagnenleistung in einer Ansicht.</p></div><div className="os-toolbar"><a className="os-btn" href="/command">← Heute</a><a className="os-btn" href="/pipeline">Pipeline</a><button className="os-btn" onClick={()=>void load()}>↻ Aktualisieren</button></div></div>

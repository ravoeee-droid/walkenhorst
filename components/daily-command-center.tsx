...
export function DailyCommandCenter({user}:{user:User}){
  ...
  const actions=useMemo(()=>{
    ...
    for(const [leadId,inbound] of latestInbound){const l=leadById.get(leadId);if(!l||["lost","won"].includes(l.status))continue;const outbound=latestOutbound.get(leadId);if(!outbound||new Date(outbound.created_at)<new Date(inbound.created_at))result.push({id:`reply-${inbound.id}`,leadId,score:1800+Math.max(l.intent_score,70),kind:"Antwort",title:"Auf Antwort reagieren",company:l.company_name,reason:inbound.subject||"Neue E-Mail-Antwort ist noch unbeantwortet.",href:l.phone?"/calls":"/dashboard",cta:l.phone?"Kontakt öffnen":"CRM öffnen",dueAt:inbound.created_at,tone:"hot"})}

    for(const q of qualifications){const l=leadById.get(q.lead_id);if(!l||["meeting","proposal","won","lost"].includes(l.status)||ageHours(q.created_at)>336)continue;result.push({id:`qual-${q.id}`,leadId:l.id,score:1700+l.intent_score,kind:"Qualifiziert",title:"Interessenten kontaktieren",company:l.company_name,reason:`Interesse: ${q.interest}${q.timeline?` · Zeitraum: ${q.timeline}`:""}`,href:l.phone?"/calls":"/pipeline",cta:l.phone?"Jetzt anrufen":"Pipeline öffnen",dueAt:q.created_at,tone:"hot"})}

    for(const d of documents){const l=leadById.get(d.lead_id);if(!l||["won","lost"].includes(l.status))continue;if(d.view_count>=3)result.push({id:`doc-${d.id}`,leadId:l.id,score:1650+d.view_count*30+l.intent_score,kind:"Angebot",title:"Angebot nachfassen",company:l.company_name,reason:`${d.title} wurde ${d.view_count}× geöffnet.`,href:l.phone?"/calls":"/proposals",cta:l.phone?"Jetzt anrufen":"Angebot öffnen",dueAt:d.last_viewed_at,tone:"hot"})}
  ...
  return <main className="os-root wh-command-page" style={{minHeight:"100vh"}}><div className="os-content" style={{maxWidth:1240,margin:"0 auto",paddingTop:32}}>
    <div className="wh-command-head">
      <div><div className="os-kicker">Heute</div><h1 className="os-title">Was jetzt wichtig ist</h1><p>Die wichtigsten Antworten, Kaufsignale und Follow-ups – automatisch nach Priorität.</p></div>
      <div className="os-toolbar"><a className="os-btn primary" href="/calls">Calls öffnen</a><button className="os-btn" disabled={busy} onClick={()=>void load()}>Aktualisieren</button></div>
    </div>

    <div className="wh-command-status" aria-label="Systemstatus">
      <span className={activeCampaigns>0?"is-ok":""}><i />{activeCampaigns} Kampagne{activeCampaigns===1?"":"n"} aktiv</span>
      <span className={unhealthyMailboxes===0&&mailboxes.length>0?"is-ok":unhealthyMailboxes>0?"is-warn":""}><i />{readyMailboxes}/{mailboxes.length} Mailboxen bereit</span>
    </div>

    {error?<div className="os-error">{error}</div>:null}{notice?<div className="os-success">{notice}</div>:null}

    <div className="os-grid os-kpis wh-command-kpis">{[[actions.length,"Offene Aktionen","gesamt"],[inboundOpen,"Antworten","offen"],[hot,"Hohe Priorität","jetzt"],[overdue,"Überfällig","Follow-ups"]].map(([v,l,s])=><div className="os-card os-kpi" key={String(l)}><div className="os-kpi-label">{l}</div><div className="os-kpi-value">{v}</div><div className="os-kpi-sub">{s}</div></div>)}</div>

    {nextAction?<section className="os-card wh-next-action">
      <div className="wh-next-label">Nächster Schritt</div>
      <div className="wh-next-main"><div><span className={`os-pill ${nextAction.tone==="hot"?"hot":nextAction.tone==="high"?"green":""}`}>{nextAction.kind}</span><h2>{nextAction.title}</h2><strong>{nextAction.company}</strong><p>{nextAction.reason}{nextAction.dueAt?` · ${fmt(nextAction.dueAt)}`:""}</p></div><div className="wh-next-actions"><a className="os-btn primary" href={nextAction.href}>{nextAction.cta}</a>{nextAction.id.startsWith("followup-")?<button className="os-btn" disabled={busy} onClick={()=>void completeFollowup(nextAction)}>Erledigt</button>:null}</div></div>
    </section>:<section className="os-card wh-next-action"><div className="wh-next-label">Heute</div><h2>Keine dringenden Aktionen</h2><p>Die priorisierte Queue ist abgearbeitet.</p><div className="os-toolbar"><a className="os-btn" href="/dashboard?section=finder">Neue Leads finden</a><a className="os-btn" href="/crm/commercial">CRM öffnen</a></div></section>}

    {actions.length>1?<section className="os-card os-section wh-command-list"><div className="os-section-head"><div><div className="os-kicker">Danach</div><h2>Weitere Prioritäten</h2></div><button className="os-btn" onClick={()=>setShowAll(v=>!v)}>{showAll?"Weniger anzeigen":"Alle anzeigen"}</button></div>
      <div className="os-grid" style={{gap:8}}>{remaining.map((a,index)=><div className="os-checkrow wh-action-row" key={a.id}><div className="os-score">{index+2}</div><span className={`os-pill ${a.tone==="hot"?"hot":a.tone==="high"?"green":""}`}>{a.kind}</span><div className="wh-action-copy"><strong>{a.title}</strong><b>{a.company}</b><small>{a.reason}{a.dueAt?` · ${fmt(a.dueAt)}`:""}</small></div><div className="os-toolbar"><a className="os-btn small" href={a.href}>{a.cta}</a>{a.id.startsWith("followup-")?<button className="os-btn small" disabled={busy} onClick={()=>void completeFollowup(a)}>Erledigt</button>:null}</div></div>)}</div>
    </section>:null}
  </div></main>;
}

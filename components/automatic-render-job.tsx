...
import { renderStudioV3Browser } from "@/lib/studio-v3-browser-render-core";
  ...
  useEffect(()=>{
    ...
    async function run(){
      try{
        ...
        setStatus("Finales HD-MP4 wird gerendert");
        setProgress(2);
        ...
        await call("complete",{bytes:result.blob.size,width:result.width,height:result.height,renderer:"webmotion-webcodecs-hd"});
        if(stopped)return;
        setProgress(100);
        setStatus("Fertig");
        setError(null);

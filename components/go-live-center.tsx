...
export function GoLiveCenter({user}:{user:User}){
 ...
 ]);if(mb.error)throw mb.error;setMailboxes((mb.data||[]) as Mailbox[]);if(!i.error)setIntegrations((i.data||[]) as Integration[]);if(!l.error)setLeads((l.data||[]) as Lead[]);if(!a.error)setAlerts((a.data||[]) as Alert[]);if(!b.error)setBounces((b.data||[]) as Bounce[]);if(!m.error)setMessages((m.data||[]) as Message[]);if(!d.error)setDuplicates(((d.data||[]) as Array<{confidence:number}>).filter(x=>x.confidence>=80).length);if(!h.error)setHealth(h.data)}catch(x){setError(x instanceof Error?x.message:"Readiness konnte nicht geladen werden")}finally{setBusy(false)}},[supabase,user.id]);useEffect(()=>{void load()},[load]);
 ...
 const required=checks.filter(c=>c.required);const requiredOk=required.filter(c=>c.ok).length;const optionalOk=checks.filter(c=>!c.required&&c.ok).length;const score=Math.round((requiredOk/required.length)*80+(optionalOk/checks.filter(c=>!c.required).length)*20);const state=requiredOk===required.length?"Go-Live bereit":"Noch nicht freigeben";
 return <main className="os-root" style={{minHeight:"100vh"}}><div className="os-content" style={{maxWidth:1400,margin:"0 auto",paddingTop:28}}>
  <div className="os-section-head"><div><div className="os-kicker">Launch Control</div><h1 className="os-title">Walkenhorst Go-Live Center</h1><p>Ein operativer Readiness-Check statt Bauchgefühl.</p></div><div className="os-toolbar"><a className="os-btn" href="/command">← Sales OS</a><button className="os-btn primary" disabled={busy} onClick={()=>void load()}>↻ Vollcheck</button></div></div>{error?<div className="os-error">{error}</div>:null}
